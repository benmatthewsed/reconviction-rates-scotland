...
# Part One: The Demographics of the Criminal Justice System {background-color="#006938"}

## Change in police recorded crime
...
dgnpop(kirwood_dat |> 
       ...
       pop="year", 
       factors=c("rate"),id_vars=c("demographic"),
       crossclassified="proportion") |> 
  dg_table()
...
# Part Three: A solution? {background-color="#006938"}
...
-   Previous regression-based approaches (e.g. [@francis2005; @cunliffe2007]; @drank2010) to correct for the problem of changing 'offender mix' are more flexible, but can't as easily assess the extent of the bias due to demographic change and are much more data intensive
...
-   We illustrate this using the methods described by [@dasguptaStandardizationDecompositionRates1993] and implemented in the `{dasguptr}` R package [@king2025] to Scottish reconvictions data from 2004-2022
...
- The by-group decomposition shows that this overall fall in reonvictions is primarily due to the under 21 group, and then the 21-25 group
...
## Next steps for the crime drop

- There has been *a lot* written about the causes of the crime drop, and some previous attempts to describe changes in the demographics of crime and use these descriptions to build explanations for why there is less crime now than in the past
...
## Non-criminological implications
- This is partly a story about the 'amalgamation fallacy' - trends for sub-groups don't always move in the same direction as headline trends
- Does this sound like it might be relevant in your area?
